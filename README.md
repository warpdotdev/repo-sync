...
3. the person who merged the source PR is assigned to review; for direct pushes (no source PR), the commit author is assigned instead.  if neither can be determined, assign to `@oncall-client-primary`
4. if the assignee doesn't respond within a configurable timeout, the PR is reassigned to `@oncall-client-primary`
...
* **reusable github workflows** — sync-private-to-public and sync-public-to-private, triggered per-commit on the default branch
