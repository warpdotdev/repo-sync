...
**decision:** each clean snapshot is generated independently (stateless), but a watermark tag tracks the last-synced source commit to know which commits to process.
...
2. **fully stateless** -- no sync state to maintain; always diff the full snapshot against public
3. **stateless snapshots + watermark tag** (chosen) -- snapshots are generated independently, but a watermark tracks which commits need processing

**justification:** each clean snapshot is independently generated from the private repo at a given commit -- no incremental state is needed to determine *what the public repo should look like*.  this preserves the self-healing property: if the watermark were lost, recovery is possible by diffing the full current snapshot against public `main`.

however, per-commit sync PRs require knowing *which commits have been synced* -- a different kind of state.  the watermark tag (a git tag in the target repo pointing to the last merged sync commit) tracks this.  this was added to support per-commit PR creation and to allow merged sync branches to be safely deleted.

the original "stateless" decision was correct for its context (snapshot generation), but the scope expanded when we added per-commit sync PRs and branch cleanup.
